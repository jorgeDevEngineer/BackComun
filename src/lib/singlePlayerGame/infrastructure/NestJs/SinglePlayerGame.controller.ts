...
import {
  StartGameRequestDto,
  SubmitGameAnswerRequestDto,
} from "../../application/dtos/SinglePlayerGameRequests.dto";
...
export class SinglePlayerGameController {
  constructor(
    ...
    private readonly StartSinglePlayerGameHandler: IHandler<
      StartSinglePlayerGameCommand,
      StartGameResponseDto
    >,

    @Inject("GetGameProgressQueryHandler")
    private readonly GetGameProgressHandler: IHandler<
      GetGameProgressQuery,
      GameProgressResponseDto
    >,

    @Inject("SubmitGameAnswerCommandHandler")
    private readonly SubmitGameAnswerHandler: IHandler<
      SubmitGameAnswerCommand,
      SubmitGameAnswerResponseDto
    >,

    @Inject("GetGameSummaryQueryHandler")
    private readonly GetGameSummaryHandler: IHandler<
      GetGameSummaryQuery,
      GameSummaryResponseDto
    >,
    ...
    try {
      ...
      if (error.message === `No se encontró el quiz de id ${body.kahootId}`) {
        throw new HttpException(
          `Quiz de id ${body.kahootId} no encontrado`,
  ...
  async submitAnswer(
    ...
    try {
      ...
      if (error.message === "La partida ya ha sido completada") {
        throw new HttpException(
          "La partida ya ha sido completada",
          HttpStatus.BAD_REQUEST
        );
      }
      if (
        error.message ===
        "La pregunta que se quiere responder ya ha sido respondida"
      ) {
        throw new HttpException(
          "La pregunta que se quiere responder ya ha sido respondida",
          HttpStatus.BAD_REQUEST
  ...
  async getSummary(
    ...
    try {
      ...
      if (error.message === `No se encontró la partida de id ${attemptId}`) {
        throw new HttpException(
          `Quiz de id ${attemptId} no encontrado`,
          HttpStatus.NOT_FOUND
        );
      }
      if (
        error.message ===
        "La partida no ha sido completada por lo que no se puede ver el resumen de partida"
      ) {
        throw new HttpException(
          "La partida no ha sido completada por lo que no se puede ver el resumen de partida",
          HttpStatus.BAD_REQUEST
  ...
  // Token validation now centralized via ITokenProvider methods used inline
}
