...
export class GetUserCompletedQuizzesDomainService {
    ...
    async execute(userId: UserIdQuizVo, criteria: QuizQueryCriteria)
      : Promise<Either<DomainException, { completedGames: SinglePlayerGame[], quizzes: Quiz[], quizAuthors: User[], totalCount: number }>> {
      ...
      return Either.makeRight({ completedGames, quizzes, quizAuthors,totalCount });
    }
  }
