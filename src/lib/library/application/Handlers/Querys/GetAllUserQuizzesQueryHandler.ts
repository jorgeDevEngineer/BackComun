...
{
  constructor(
    private readonly getQuizDService: GetAllUserQuizzesDomainService
  ) {}

  async execute(
    command: GetAllUserQuizzes
  ): Promise<
    Either<DomainException, QueryWithPaginationResponse<QuizResponse>>
  > {
    ...
    const dServiceResponse = await this.getQuizDService.execute(
      userId,
      criteria
    );
    ...
    const data: QuizResponse[] = quizzes.map((quiz: Quiz) =>
      toQuizResponse(quiz, user)
    );
